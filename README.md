...
```text
src/
├── factory_method/       # Factory Method implementation (Transport & Logistics)
│   ├── Transport.java
│   ├── Truck.java
│   ├── Ship.java
│   ├── Logistics.java
│   ├── RoadLogistics.java
│   └── SeaLogistics.java
├── abstract_factory/     # Abstract Factory implementation (UI components & factories)
│   ├── Button.java
│   ├── Checkbox.java
│   ├── WindowsButton.java
...
Enter the delivery mode and UI platform in the console prompt.

Option 2: Command Line
Compile all source files and run the main entry point:

javac -d bin src/factory_method/*.java src/abstract_factory/*.java src/app/*.java src/Main.java
java -cp bin Main
